# Roadmap

## Phase 1 — Repository and Spark foundation

- [x] Create Poetry project
- [x] Add package layout under `src/transaction_risk`
- [x] Add Spark session factory
- [x] Add local configuration files
- [x] Add Docker Compose for Spark
- [x] Add Makefile commands
- [x] Add CI workflow

## Phase 2 — Lakehouse ingestion

- [x] Define PaySim schema
- [x] Read raw CSV with strict schema
- [x] Write bronze Parquet table
- [x] Clean and deduplicate silver transactions
- [x] Add data quality checks
- [x] Add optional Delta Lake support
- [x] Add IEEE-CIS ingestion module

## Phase 3 — Feature engineering

- [x] Transaction-level features
- [x] Balance-consistency features
- [x] Entity-level aggregation features
- [x] Temporal window features
- [x] Spark-native graph-derived features
- [x] Add device / identity feature group for IEEE-CIS
- [x] Add feature store metadata

## Phase 4 — Modelling

- [x] Rule-based baseline
- [x] Logistic regression pipeline
- [x] Random forest pipeline
- [x] Gradient-boosted tree pipeline
- [x] Temporal split utility
- [x] PR-AUC and ROC-AUC evaluation
- [x] Precision@K and recall@K
- [x] Threshold selection
- [x] Add cost-sensitive threshold optimization
- [x] Add calibrated probability output

## Phase 5 — Monitoring and operations

- [x] Population stability index helper
- [x] Alert summary report
- [x] Model card template
- [x] Local streaming scoring job
- [x] Batch scoring job for materialized tables
- [x] Local model registry with versioned metadata
- [x] Drift and performance monitoring reports (JSON and Markdown)
- [x] Add Evidently-style HTML report export (dependency-free)
- [x] Add lightweight expectation checks (Great Expectations-style)
- [x] Add Dagster example pipeline (optional dependency group)

## Phase 6 — Portfolio polish

- [x] Add architecture diagram
- [x] Add executable notebook content
- [x] Add benchmark table in README (generated by `make benchmark`)
- [x] Add demo figure script with optional GIF (`make demo-artifacts`)
- [x] Add blog-style walkthrough (docs/walkthrough.md)

## Phase 7 — Operational maturity

- [ ] Make the default `make test` / `pytest` path faster and more deterministic on local Spark, especially on Windows
- [ ] Split lightweight unit coverage from Spark-heavy integration coverage more aggressively so routine validation stays practical
- [ ] Document and validate supported environment combinations explicitly (`Python`, `PySpark`, `Java`, `HADOOP_HOME` / `winutils`)
- [ ] Add a reproducible end-to-end smoke test that exercises sample data, ingestion, features, training, and scoring
- [ ] Extend evaluation with clearer fraud-operations views: cost curves, review-capacity simulation, and segment-level threshold analysis
- [ ] Model delayed-label realities more explicitly in evaluation and monitoring workflows
- [ ] Add calibration and alert-quality drift views to complement PSI-style feature drift
- [ ] Strengthen batch-scoring and artifact lifecycle ergonomics, including clearer version promotion / rollback flows
- [ ] Expand the orchestration and deployment story beyond local demos so the repo shows a more production-like execution path
